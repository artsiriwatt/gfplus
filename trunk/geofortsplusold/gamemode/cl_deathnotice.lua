--credit SteveUK (headshot symbols)

CreateConVar( "hud_deathnotice_time", "6", FCVAR_REPLICATED )

// These are our kill icons
local Color_Icon = Color( 255, 200, 200, 255 ) 
local NPC_Color = Color( 255, 200, 200, 255 )
...
// cap d
killicon.AddFont( "headshot", "CSKillIcons", "D", Color( 255, 200, 200, 255 ) )
...
local function RecvPlayerKilledByPlayer( message )
	...
	local hitbox	= message:ReadShort();
			
	GAMEMODE:AddDeathNotice( attacker:Name(), attacker:Team(), inflictor, victim:Name(), victim:Team(), hitbox )
...
---------------------------------------------------------*/
function GM:AddDeathNotice( Victim, team1, Inflictor, Attacker, team2, hitbox )
...
local function DrawDeath( x, y, death, hud_deathnotice_time )
	...
	local killerleftoffset
	
	if( death.hitbox == HITGROUP_HEAD ) then
		killicon.Draw( x - (w/2) - 25, y, "headshot", alpha )
		killerleftoffset = 50
	else
		killerleftoffset = 16
	end
	...
	// Draw KILLER
	if (death.left) then
		draw.SimpleText( death.left, 	"ChatFont", x - (w/2) - killerleftoffset, y, 		death.color1, 	TEXT_ALIGN_RIGHT )
	end
